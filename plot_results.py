...
def plot_training_results():
    ...
    if len(rewards) > 10:
        ...
        ax1.plot(range(9, 9+len(smoothed)), smoothed, color='darkblue', linewidth=2, label='Moving Avg')
    ...
    ax4.axvline(np.mean(rewards), color='red', linestyle='--', label=f'Mean: {np.mean(rewards):.2f}')
