...
2. [Environment](#2-environment-highway-v0)
...
**Key result:** PPO achieves 35.2 avg reward vs 6.1 for random (5.8× better), with only a 2% crash rate.

---

## 2. Environment: Highway-v0

### What it is
`highway-v0` is from the [highway-env](https://github.com/eleurent/highway-env) package — a suite of environments for autonomous driving research. It is **not** part of standard OpenAI Gym.
...
2. Creates 4 parallel `highway-v0` environments
...
### `evaluate.py`
**What it does:**
- Loads `ppo_highway_final.zip`
- Runs 50 episodes with trained PPO (deterministic=True: no exploration)
- Runs 50 episodes with random agent
- Prints side-by-side statistics table

**Key metric:** `improvement = (ppo_mean - random_mean) / random_mean * 100`

### `compare_agents.py`
**What it does:**
- Loads both PPO and A2C models
- Evaluates each for 50 episodes
- Evaluates random agent for 50 episodes
- Prints comprehensive 3-way comparison table
- Shows which is more stable (std dev), safer (crash rate), better (mean reward)
...
2. Creates high-res `highway-v0` environment (1920×600)
...
# Step 5: Evaluate PPO vs Random (1 minute)
python evaluate.py

# Step 6: Record video (2 minutes)
...
|------|-------------|---------|
| `models/ppo_highway_final.zip` | train_ppo.py | Frozen PPO weights |
| `models/a2c_highway_final.zip` | train_a2c.py | Frozen A2C weights |
| `models/best_model.zip` | EvalCallback | Best checkpoint during training |
...
| `results/evaluations.npz` | EvalCallback | Evaluation scores over training |
...
| `results/videos/highway_ppo_annotated.mp4` | record_video.py | 60s annotated demo video |
